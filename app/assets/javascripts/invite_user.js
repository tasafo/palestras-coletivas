    ...
    if (user_desc != "") {
        ...
        if (!found) {
            $('#table_users > tbody:last').append(
                '<tr id="row_' + user_id + '"><td>' + user_desc + '<input type="hidden" name="users[]" value="' + user_id + '" /></td><td><input type="button" class="btn btn-warning" id="user_id_' + user_id + '" onclick="remove_user(\'' + user_id + '\')" value="' + $("#remove").text() + '" /></td></tr>'
            );
        }
    }
}
